...
oneHot_encode = joblib.load('onehot_encode.pkl')
...
def main():
    ...
    geography = st.number_input("location", ["France", "Germany", "Spain"])
    ...
    geo = df[['Geography']]
    location = pd.DataFrame(oneHot_encode.transform(geo).toarray(),columns = oneHot_encode.get_feature_names_out())
